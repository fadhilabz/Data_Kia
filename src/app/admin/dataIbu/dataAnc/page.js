// app/admin/dataAnc/page.js
// Halaman Utama Admin Data ANC Kota Baubau
// Dibuat sangat ringkas (<100 baris) sebagai wrapper modul-modul terpisah.

"use client";

import { useState } from "react";
import { useAncPeriod } from "@/hooks/hooksIbu/anc/useAncPeriod";
import { useAncExport } from "@/hooks/hooksIbu/anc/useAncExport";
import PeriodeBulanCard from "@/components/shared/PeriodeBulanCard";
import AncTable from "@/components/comIbu/anc/table/AncTable";
...
export default function DataAncPage() {
  ...
  // Perbaikan 1: Destructure periodStatusesMap, activePeriodData, & setSelectedMonth dari hook
  // Perbaikan 2: Hapus hardcoded parameter "01" agar auto-sync dengan Firestore
  const {
    selectedMonth,
    setSelectedMonth,
    selectedYear,
  ...
  const { handleExportExcel, exporting } = useAncExport();
      ...
      <div className="flex justify-center items-center h-screen bg-gray-50">
        <div className="text-center space-y-3">
          ...
          <p className="text-gray-600 font-medium text-xs">
            Memuat Laporan Data ANC 78 Kolom...
          </p>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-gray-800 tracking-tight flex items-center gap-2">
            ...
            Rekapitulasi Data ANC 17 Puskesmas Kota Baubau
          </h1>
      ...
      {/* Modular ANC Spreadsheet Table Component */}
      <AncTable
