// app/admin/dataSdm/page.js
"use client";

import { useState } from "react";
import { useSdmPeriod } from "@/hooks/sdm/useSdmPeriod";
import { useSdmExport } from "@/hooks/sdm/useSdmExport";
import PeriodeBulanCard from "@/components/shared/PeriodeBulanCard";
import SdmTable from "@/components/sdm/table/SdmTable";
import { formatPeriode } from "@/constants/periode";

export default function DataSdmPage() {
  ...
  const { handleExportExcel, exporting } = useSdmExport();
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-gray-50">
        <div className="text-center space-y-3">
          ...
          <p className="text-gray-600 font-medium text-xs">Memuat Laporan Data SDM...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-gray-800 tracking-tight flex items-center gap-2">
            ...
            Rekapitulasi Data SDM & Fasilitas Kesehatan Kota Baubau
          </h1>
          <p className="text-xs text-gray-500 mt-0.5">
            ...
            {" — "}
            <span className="text-blue-600">data kumulatif (sudah termasuk bulan sebelumnya)</span>
          </p>
        </div>
      ...
      <SdmTable reportList={reportList} searchQuery={searchQuery} zoomLevel={zoomLevel} />
    </div>
  );
}