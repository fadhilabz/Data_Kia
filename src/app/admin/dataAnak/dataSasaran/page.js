// app/admin/dataSasaranAnak/page.js
//
// Modul "PL Balita" (Data Sasaran Balita) — SATU FILE lengkap.
// Input dikunci (disabled) kalau periode bulan yang dipilih BELUM DIBUKA
// oleh admin lewat menu "Periode Pelaporan" (periodeSudahDibuka === false).
...
const SASARAN_ANAK_FIELDS = [
  { key: "sasaranKelahiranHidup", label: "Sasaran Kelahiran Hidup" },
  { key: "sasaranAnakBalita1259", label: "Sasaran Anak Balita (12-59 Bulan)" },
  { key: "sasaranBalita059", label: "Sasaran Balita (0-59 Bulan)" },
];
...
function useSasaranAnakPeriode() {
  const {
    selectedYear,
    openedMonths,
  ...
  const loadRows = useCallback(async (year, month) => {
    ...
    try {
      ...
      const collectionName = getSasaranAnakCollectionName(year, month);
      ...
      console.error("useSasaranAnakPeriode loadRows error:", err);
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const saveRow = async (row) => {
    ...
    setSavingId(row.id);
    try {
      const collectionName = getSasaranAnakCollectionName(selectedYear, selectedMonth);
      ...
      console.error("useSasaranAnakPeriode saveRow error:", err);
      return false;
    } finally {
      setSavingId(null);
    }
...
export default function DataSasaranAnakPage() {
  const {
    selectedMonth,
    setSelectedMonth,
    selectedYear,
    ...
    <div className="p-6 min-h-screen bg-gray-100 space-y-5">
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
        <h1 className="text-xl font-bold text-gray-800 tracking-tight flex items-center gap-2">
          <span className="material-symbols-outlined text-emerald-600">child_friendly</span>
          Data Sasaran Balita (PL Balita) per Puskesmas
        </h1>
      ...
      {!periodeSudahDibuka && selectedMonth && (
        <div className="bg-amber-50 border border-amber-300 rounded-lg p-3 text-sm text-amber-800">
          ...
          Sasaran Balita untuk bulan ini.
        </div>
      )}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-auto">
          <table className="border-collapse w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 && (
                <tr>
                  <td colSpan={3 + SASARAN_ANAK_FIELDS.length} className="border border-gray-300 px-3 py-6 text-center text-gray-400">
