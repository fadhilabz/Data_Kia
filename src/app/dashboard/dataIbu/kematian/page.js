...
import { useKematianPeriode } from "@/lib/ibu/kematian/useKematianPeriode";
import { useKematianFormData } from "@/lib/ibu/kematian/useKematianFormData";
import { STATUS_DRAFT } from "@/lib/ibu/kematian/kematianConfig";

import PeriodeBulanCard from "@/components/shared/PeriodeBulanCard";
import StepKematian from "@/components/comIbu/kematian/form/StepKematian";
import KematianReportTablePreview from "@/components/comIbu/kematian/table/KematianReportTablePreview";
...
export default function FormKematianPage() {
  ...
  const onFinalSubmit = async () => {
    ...
    if (ok) {
      alert(
        `Laporan Kematian Ibu untuk ${formatPeriode(selectedYear, selectedMonth)} berhasil disimpan dan ditandai selesai!`,
      );
      ...
      <div className="min-h-screen flex items-center justify-center bg-surface text-on-surface">
        <div className="text-center space-y-3">
          ...
          <p className="text-on-surface-variant font-medium text-xs">
            Memuat Halaman Form Kematian Ibu...
          </p>
        </div>
      </div>
    );
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="font-bold text-lg text-primary">
            Form Kematian Ibu — {puskesmasName}
          </h1>
