...
export function useSdmFormData({ selectedYear, selectedMonth, isReadOnly }) {
  ...
  const [diisiOtomatisDariBulanLalu, setDiisiOtomatisDariBulanLalu] = useState(false);

  const currentCollectionName = getSdmCollectionName(selectedYear, selectedMonth);
    ...
    async (puskId, targetYear, targetMonth, profileForInit, canEdit) => {
      ...
      try {
        ...
        if (canEdit && profileForInit) {
          ...
          try {
            const prevSnap = await getDoc(getSdmReportRef(prevCollection, puskId));
            if (prevSnap.exists()) {
  ...
  const saveToFirestore = useCallback(
    async (dataToSave, statusReport = STATUS_DRAFT) => {
      ...
      try {
        const reportRef = getSdmReportRef(currentCollectionName, userProfile.puskesmasId);
