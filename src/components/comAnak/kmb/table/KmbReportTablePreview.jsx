// components/comAnak/kmb/table/KmbReportTablePreview.jsx
"use client";

import { useState, useEffect } from "react";
import { collection, getDocs, getDoc, doc } from "firebase/firestore";
import { db } from "@/lib/firebase";
import { getKmbCollectionName, namaBulan, STATUS_FIELD, STATUS_SUBMITTED } from "@/lib/libAnak/kmb/kmbConfig";
import KmbTable from "./KmbTable";

export default function KmbReportTablePreview({ userProfile, selectedMonth, selectedYear }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const collectionName = getKmbCollectionName(selectedYear, selectedMonth);
        ...
        console.error("Error loading KMB preview table:", err);
        setReportList([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <KmbTable reportList={displayedList} searchQuery={searchQuery} />
    </div>
  );
}