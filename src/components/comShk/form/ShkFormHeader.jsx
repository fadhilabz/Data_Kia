// components/comShk/form/ShkFormHeader.jsx
"use client";

export default function ShkFormHeader({ puskesmasName, sasaranBbl, autoSaveStatus, isEditable, periodLabel }) {
  return (
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-surface-container-lowest border border-outline-variant rounded-xl p-4">
      <div>
        <h1 className="text-xl font-bold text-on-surface tracking-tight">
          Form Pelaporan SHK (Skrining Hipotiroid Kongenital)
        </h1>
        <p className="text-xs text-on-surface-variant mt-0.5 flex items-center gap-1.5">
          <span className="material-symbols-outlined text-sm">local_hospital</span>
          {puskesmasName}
          <span>•</span>
          Sasaran BBL: <strong>{sasaranBbl}</strong> orang
        </p>
      </div>
