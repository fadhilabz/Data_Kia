// components/comShk/table/ShkReportTablePreview.jsx
"use client";

import { useState, useEffect } from "react";
import { collection, getDocs, getDoc, doc } from "firebase/firestore";
import { db } from "@/lib/firebase";
import { getShkCollectionName, namaBulan, TEMPLATE_KOLOM_SHK, STATUS_FIELD, STATUS_SUBMITTED } from "@/lib/libShk/shkConfig";
import ShkTable from "./ShkTable";

export default function ShkReportTablePreview({ userProfile, selectedMonth, selectedYear }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const collectionName = getShkCollectionName(selectedYear, selectedMonth);
        ...
        if (isPetugas && userProfile?.puskesmasId) {
          ...
          if (pkmSnap.exists()) {
            ...
            pkmMap.set(pkmSnap.id, { id: pkmSnap.id, nama: pData.nama || pkmSnap.id, sasaranBbl: pData.sasaranBbl || 0 });
          }
          ...
          puskesmasSnap.forEach((docSnap) => {
            ...
            pkmMap.set(docSnap.id, { id: docSnap.id, nama: pData.nama || docSnap.id, sasaranBbl: pData.sasaranBbl || 0 });
          });
        ...
        pkmMap.forEach((pkmInfo, pkmId) => {
          ...
          combined.push({
            ...
            sasaranBbl: Number(repData.sasaranBbl ?? pkmInfo.sasaranBbl ?? 0),
        ...
        console.error("Error loading SHK preview table:", err);
        setReportList([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <ShkTable reportList={displayedList} searchQuery={searchQuery} />
    </div>
  );
}