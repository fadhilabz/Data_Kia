// components/anc/AncStatusBanners.js
export default function AncStatusBanners({ periodDocExists, isReadOnly, periodLabel }) {
  if (!periodDocExists) {
    return (
      <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl text-amber-900 text-xs flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-0.5 opacity-90">
            Dokumen pelaporan untuk <strong>{periodLabel}</strong> belum pernah diinisialisasi oleh Admin Dinas Kesehatan.
          </p>
        </div>
      </div>
    );
