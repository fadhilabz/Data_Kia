...
import { namaBulan } from '@/lib/sdm/sdmConfig';
...
export function useSdmExport() {
  ...
  const handleExportExcel = async (reportList, { bulan, tahun }) => {
    ...
    try {
      ...
      const headerRow = ['NO', 'NAMA PUSKESMAS', ...semuaKolom.map((f) => `${f.group} - ${f.label}`)];

      const dataRows = reportList.map((row, idx) => [
        idx + 1,
        row.namaPuskesmas || row.id,
        ...semuaKolom.map((f) => (f.hitung ? f.hitung(row) : Number(row[f.key] || 0))),
      ]);
      ...
      const totalRow = [
        ...
        'TOTAL',
        ...semuaKolom.map((f) =>
          reportList.reduce((sum, row) => sum + (f.hitung ? f.hitung(row) : Number(row[f.key]) || 0), 0)
        ),
      ];
