// hooks/hookAnak/kmb/useKmbPeriod.js
'use client';
...
export function useKmbPeriod() {
  ...
  const loadReportList = useCallback(async (year, month) => {
    ...
    setLoading(true);
    try {
      const collectionName = getKmbCollectionName(year, month);
      ...
      const rows = await Promise.all(
        puskesmasSnap.docs.map(async (pDoc) => {
          ...
          return { id: pDoc.id, nama: pData.nama || pDoc.id, kasusList: reportData.kasusList || [], ...reportData };
        })
      );
      rows.sort((a, b) => (a.nama || '').localeCompare(b.nama || ''));
      setReportList(rows);
    } catch (err) {
      console.error('useKmbPeriod loadReportList error:', err);
      setReportList([]);
    } finally {
      setLoading(false);
    }
