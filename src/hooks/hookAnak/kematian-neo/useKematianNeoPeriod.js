// hooks/kematian-neo/useKematianNeoPeriod.js
'use client';
...
export function useKematianNeoPeriod() {
  const {
    selectedYear,
    openedMonths,
  ...
  const loadReportList = useCallback(async (year, month) => {
    ...
    setLoading(true);
    try {
      const collectionName = getKematianNeoCollectionName(year, month);
      ...
      const rows = await Promise.all(
        puskesmasSnap.docs.map(async (pDoc) => {
          ...
          return { id: pDoc.id, nama: pData.nama || pDoc.id, ...reportData };
        })
      );
      rows.sort((a, b) => (a.nama || '').localeCompare(b.nama || ''));
      setReportList(rows);
    } catch (err) {
      console.error('useKematianNeoPeriod loadReportList error:', err);
      setReportList([]);
    } finally {
      setLoading(false);
    }
